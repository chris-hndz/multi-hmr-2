...
public class SMPLXAligner3 : MonoBehaviour
{
    ...
    public Transform joint3dParent;
    ...
    void AlignSMPLX()
    {
        foreach (HumanParams human in parameters.humans)
        {
            ...
            if ((human.joints_2d != null)&&(human.joints_3d != null)) {
                CalculateJointsScreenPositions(human.joints_2d);
                AlignWithPelvis3D(smplxInstance, human.joints_3d[0]);//, (float)MODEL_IMAGE_SIZE, parameters.image_width, parameters.image_height, human.translation[2]);
            }

            if (human.joints_3d != null)
            {
                CreateJoints3D(human.joints_3d, joint3dParent);
                AlignWithPelvis2D(joint3dParent);
            }
        }
    }
    ...
    void AlignWithPelvis2D(Transform pelvis3D)
    {
        // Obtener las coordenadas 2D de la pelvis (asumiendo que es el primer joint)
        Vector2 pelvis2D = new Vector2 (jointsScreenPositions[0].x, jointsScreenPositions[0].y);
    ...
    void AlignWithPelvis3D(GameObject smplxInstance, float[] joint_3d_pelvis) //, float model_img_size, float image_width, float image_height, float translationZ)
    {
        ...
        Vector2 pelvis2D = new Vector2 (jointsScreenPositions[0].x, jointsScreenPositions[0].y);
        Vector3 pelvis3DWorld = alignmentCamera.ScreenToWorldPoint(new Vector3(pelvis2D.x, pelvis2D.y, joint_3d_pelvis[2]));
        ...
        Debug.Log($"Pelvis 3D Z: {joint_3d_pelvis[2]} Pelvis 2D: {pelvis2D}, 3D World: {pelvis3DWorld}");
    }
    void AlignWithPelvis2D(GameObject smplxInstance, float[][] joints_2d, float model_img_size, float image_width, float image_height, float translationZ)
    {
        Transform pelvisTransform = smplxInstance.GetComponent<SMPLX>().JointTransforms[_smplxJointNames[0]];
        //Debug.Log($"pelvisTransform: {pelvisTransform}");
        Vector3 relativePositionPelvis = smplxInstance.transform.position - pelvisTransform.position;
        ...
        // Obtener las coordenadas 2D de la pelvis (asumiendo que es el primer joint)
        Vector2 pelvis2D = new Vector2 (jointsScreenPositions[0].x, jointsScreenPositions[0].y);
        Vector3 newPelvis3DPosition = alignmentCamera.ScreenToWorldPoint(new Vector3(pelvis2D.x, pelvis2D.y, translationZ));
        Vector3 pelvis3DWorld = new Vector3( newPelvis3DPosition.x, newPelvis3DPosition.y, newPelvis3DPosition.z);

        // Ajustar la posición del modelo SMPLX
        Vector3 offset = pelvis3DWorld - smplxInstance.transform.position;
        smplxInstance.transform.position += offset + relativePositionPelvis;

        Debug.Log($"Pelvis 2D: {pelvis2D}, 3D World: {pelvis3DWorld}");
    }
    ...
    void CreateJoints3D(float[][] joints3D, Transform parent)
    {
        parent.position = new Vector3(-joints3D[0][0], joints3D[0][1], joints3D[0][2]);
        foreach (float[] joint in joints3D)
        {
            GameObject jointObject = Instantiate(jointPrefab, parent);
            //jointObject.transform.localPosition = new Vector3(-joint[0], joint[1], joint[2]);
        ...
        parent.rotation = Quaternion.Euler(0, 0, 180);
